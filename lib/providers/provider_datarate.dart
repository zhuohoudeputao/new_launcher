...
DataRateConverterModel dataRateConverterModel = DataRateConverterModel();

MyProvider providerDataRateConverter = MyProvider(
...
Future<void> _provideActions() async {
  Global.addActions([
    MyAction(
      ...
      action: () => dataRateConverterModel.refresh(),
...
Future<void> _initActions() async {
  dataRateConverterModel.init();
  Global.infoModel.addInfoWidget(
      "DataRateConverter",
      ChangeNotifierProvider.value(
          value: dataRateConverterModel,
          builder: (context, child) => DataRateConverterCard()),
      title: "Data Rate Converter");
}

Future<void> _update() async {
  dataRateConverterModel.refresh();
}

const Map<String, String> dataRateUnits = {
  'bps': 'bps',
  'Kbps': 'Kbps',
  'Mbps': 'Mbps',
  'Gbps': 'Gbps',
...
class DataRateConversionHistory {
  ...
  String get inputSymbol => dataRateUnits[inputUnit] ?? inputUnit;
  String get outputSymbol => dataRateUnits[outputUnit] ?? outputUnit;
}
...
class DataRateConverterModel extends ChangeNotifier {
  ...
  List<DataRateConversionHistory> get history => List.unmodifiable(_history);
  bool get hasHistory => _history.isNotEmpty;
  List<String> get availableUnits => dataRateUnits.keys.toList();

  void init() {
    _isInitialized = true;
    _convert();
    Global.loggerModel.info("DataRateConverter initialized", source: "DataRateConverter");
    notifyListeners();
  }

  void refresh() {
    notifyListeners();
    Global.loggerModel.info("DataRateConverter refreshed", source: "DataRateConverter");
  }
  ...
  void swapUnits() {
    ...
    _convert();
    notifyListeners();
    Global.loggerModel.info("DataRate units swapped", source: "DataRateConverter");
  }
  ...
  void clear() {
    ...
    _convert();
    notifyListeners();
    Global.loggerModel.info("DataRateConverter cleared", source: "DataRateConverter");
  }
  ...
  static double convert(double value, String fromUnit, String toUnit) {
    ...
    final bpsValue = value * (bitsPerSecondPerUnit[fromUnit] ?? 1.0);
    return bpsValue / (bitsPerSecondPerUnit[toUnit] ?? 1.0);
  }
  ...
  void addToHistory() {
    ...
    _history.insert(0, DataRateConversionHistory(
      inputValue: input,
    ...
    notifyListeners();
    Global.loggerModel.info("DataRate conversion added to history", source: "DataRateConverter");
  }

  void clearHistory() {
    _history.clear();
    notifyListeners();
    Global.loggerModel.info("DataRateConverter history cleared", source: "DataRateConverter");
  }

  void useHistoryEntry(DataRateConversionHistory entry) {
...
class DataRateConverterCard extends StatefulWidget {
  @override
  State<DataRateConverterCard> createState() => _DataRateConverterCardState();
}

class _DataRateConverterCardState extends State<DataRateConverterCard> {
  bool _showHistory = false;

  @override
  Widget build(BuildContext context) {
    final converter = context.watch<DataRateConverterModel>();
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 12),
              Text("Data Rate Converter: Loading..."),
            ],
          ),
        ),
      );
  ...
  Widget _buildConverterView(DataRateConverterModel converter) {
    return Row(
      children: [
  ...
  Widget _buildInputSection(DataRateConverterModel converter) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
  ...
  Widget _buildOutputSection(DataRateConverterModel converter) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
  ...
  Widget _buildUnitDropdown(DataRateConverterModel converter, String currentUnit, bool isInput) {
  ...
  Widget _buildValueField(DataRateConverterModel converter, bool isInput) {
    final colorScheme = Theme.of(context).colorScheme;
  ...
  Widget _buildHistoryView(DataRateConverterModel converter) {
    return Container(
  ...
  Future<void> _showClearHistoryConfirmation(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Clear all data rate conversion history?"),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      context.read<DataRateConverterModel>().clearHistory();
    }
  }
}