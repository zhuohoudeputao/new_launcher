...
class _RockPaperScissorsCardState extends State<RockPaperScissorsCard> {
  ...
  Widget _buildHistoryList(BuildContext context, RockPaperScissorsModel rps) {
    if (!rps.hasHistory) {
      return Padding(
    ...
    return Column(
      children: rps.history.take(5).map((entry) => 
        _buildHistoryEntry(context, rps, entry)
      ).toList(),
    );
  }

  Widget _buildHistoryEntry(BuildContext context, RockPaperScissorsModel rps, RPSGameEntry entry) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Text(rps.getChoiceEmoji(entry.playerChoice), style: TextStyle(fontSize: 16)),
          SizedBox(width: 4),
          Text("vs", style: TextStyle(fontSize: 10)),
          SizedBox(width: 4),
          Text(rps.getChoiceEmoji(entry.computerChoice), style: TextStyle(fontSize: 16)),
          SizedBox(width: 8),
          Text(
            rps.getResultText(entry.result),
            style: TextStyle(
              fontSize: 12,
              color: rps.getResultColor(entry.result, context),
